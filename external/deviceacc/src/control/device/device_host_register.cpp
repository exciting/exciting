...
public:
    ...
    ~device_host_register() {
        for (auto &entry : this->rg) {
            if (std::get<idx_device_ptr>(entry.second) == nullptr) {
                omp_target_free(std::get<idx_host_ptr>(entry.second), std::get<idx_device_id>(entry.second));
            } else {
                auto info = omp_target_disassociate_ptr(std::get<idx_device_ptr>(entry.second), std::get<idx_device_id>(entry.second));
                omp_target_free(std::get<idx_host_ptr>(entry.second), std::get<idx_device_id>(entry.second));
            }
        }
    }
    ...
    void associate(identifier id, void* host_ptr) {
        ...
        if (omp_target_is_present(host_ptr, std::get<idx_device_id>(mrg))) {
            throw std::runtime_error("Error in associate: " + id + " host pointer is already associated to a device pointer");
        }

        auto info = omp_target_associate_ptr(host_ptr, std::get<idx_host_ptr>(mrg), std::get<idx_obj_size>(mrg), 0, std::get<idx_device_id>(mrg));
    ...
    void disassociate(identifier id) {
        ...
        auto info = omp_target_disassociate_ptr(std::get<idx_device_ptr>(mrg), std::get<idx_device_id>(mrg));
        
        if (info != 0) {
            throw std::runtime_error("Error in disassociate: " + id + ". Call to omp_target_disassociate_ptr failed");
        }

        std::get<idx_device_ptr>(mrg) = nullptr;
    }
    ...
    void host_to_device(identifier id) {
        ...
        if (std::get<idx_device_ptr>(mrg) == nullptr || std::get<idx_host_ptr>(mrg) == nullptr) {
            throw std::runtime_error("Error in host_to_device: " + id + " some pointers are not associated");
        }

        auto info = omp_target_memcpy(std::get<idx_host_ptr>(mrg), std::get<idx_device_ptr>(mrg), std::get<idx_obj_size>(mrg), 0, 0, std::get<idx_device_id>(mrg), this->host_id);
    ...
    void device_to_host(identifier id) {
        ...
        auto info = omp_target_memcpy(std::get<idx_device_ptr>(mrg), std::get<idx_host_ptr>(mrg), std::get<idx_obj_size>(mrg), 0, 0, this->host_id, std::get<idx_device_id>(mrg));
    ...
    void remove(identifier id) {
        ...
        auto& mrg = rg[id];
        if (std::get<idx_device_ptr>(mrg) == nullptr) {
            omp_target_free(std::get<idx_host_ptr>(mrg), std::get<idx_device_id>(mrg));
        } else {
            auto info = omp_target_disassociate_ptr(std::get<idx_device_ptr>(mrg), std::get<idx_device_id>(mrg));
            if (info != 0) {
    ...
    void* get_device_ptr(identifier id) {
        ...
        auto &mrg = rg[id];
        return std::get<idx_host_ptr>(mrg);
    }
