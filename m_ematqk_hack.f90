...
module m_ematqk_hack
  ...
  integer(4) :: apwmaxsize
  ! maximum number of local orbitals
  integer(4) :: lomaxsize
  ! ??? 
  integer(4), allocatable :: apwsize, losize
  ...
  contains
    ...
    subroutine ematqk_setup(kqg, gqg, lray)
      ...
      ! Generate spherical harmonics and structure factors
      if( allocated( ylmgq)) deallocate( ylmgq)
      allocate( ylmgq( lmmaxapw, Gqgrid%ngkmax, kqgrid%nkpt))
      if( allocated( sfacgq)) deallocate( sfacgq)
      allocate( sfacgq( Gqgrid%ngkmax, natmtot, kqgrid%nkpt))
      do iq = 1, kqgrid%nkpt 
        do igq = 1, Gqgrid%ngk( 1, iq)                            
          Call sphcrd( Gqgrid%vgkc( :, igq, 1, iq), r, tp)
          Call genylm( lrayleigh, tp, ylmgq( 1, igq, iq))
        end do
        Call gensfacgp( Gqgrid%ngk( 1, iq), Gqgrid%vgkc( :, :, 1, iq), Gqgrid%ngkmax, sfacgq( :, :, iq)) 
      end do
...
!
    ...
    use omp_lib
      ...
      integer :: is, ia, ias
      integer :: lmax1, lmax3, ikt, zmsize
      ...
      do is = 1, nspecies
        do ia = 1, natoms(is)
          ...
          call zgemm('n', 'n', bc%n2, apwsize(is) + losize(is), apwsize(is) + losize(is),&
            & zone, cmtfun( :, :, ias), bc%n2, integrals( :, :, ias), apwmaxsize + lomaxsize,&
            & zzero, zm, bc%n2)
          call zgemm('n', 't', bc%n1, bc%n2, apwsize(is) + losize(is),&
            & prefactor, cmtfun0( :, :, ias), bc%n1, zm, bc%n2, zone, emat, bc%n1)
                      ...
                      do m3 = -l3, l3
                        ...
                        do io2 = 1, apword( l3, is)
                          ...
                          intrgaa(lm1, io1, lm3, io2) =&
                            ...
                            &* conjg(ylmgq(lm2, igq, iq)) * gaunt( l1, l2, l3, m1, m2, m3) 
                        end do
                      end do
                    end do
