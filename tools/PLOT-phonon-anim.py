...
print "\nThe supercell has dimensions:",n1,n2,n3
...
nsteps = input("\nPlease, specify the number of steps in the animation >>>> ")
print "\n---------------------------------------------------------------------\n"
...
speciespath = tree.xpath('/input/structure/@speciespath')[0]
for isp in range(nspec):
   for iat in range(natom[isp]):
      s = '/input/structure/species[' + str(isp+1) + ']/atom[' + str(iat+1) + ']/@coord'
      atcoord[isp][iat][0],atcoord[isp][iat][1],atcoord[isp][iat][2] = [float(x) for x in tree.xpath(s)[0].split()]
   ...
   for imode in range(3*natmtot):
      ...
      # plot all ions in the cell
                  for isp in range(nspec):
                     for iat in range(natom[isp]):
                        for ipol in range(3):
                           dispr[isp][iat][ipol] = evr[iqpt][imode][isp][iat][ipol] / sqrt(mass[isp]) * scaling
                           dispi[isp][iat][ipol] = evi[iqpt][imode][isp][iat][ipol] / sqrt(mass[isp]) * scaling
      # r(R) = R + d + u(R)
                        ...
                        s = str(int(on[isp])) + '   ' + str(datcoord[isp][iat][0]) + '   ' + str(datcoord[isp][iat][1]) + '   ' + str(datcoord[isp][iat][2]) + \
                                                '   ' +  str(0.) + '   ' + str(0.) + '   ' + str(0.) + '\n'
                        f.write(s)
                        s = str(symb[isp]) + '   ' + str(datcoord[isp][iat][0]) + '   ' + str(datcoord[isp][iat][1]) + '   ' + str(datcoord[isp][iat][2]) + '\n'
                        f2.write(s)
      f.close()
      f2.close()
