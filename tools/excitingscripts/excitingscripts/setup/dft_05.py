...
def setup_dft_05(input_file: Union[str, pathlib.Path], r_cut_min: float, r_cut_max: float,
    ...
    """Generate a set of input files varying DFT-1/2 shell "cut" values.

    :param input_file: Input file.
    :param r_cut_min: Minimum r_cut value.
    :param r_cut_max: Maximum r_cut value.
    :param number_r_cut_steps: Number of r_cut values for which input files are generated by varying the shell
    attribute "cut".
    ...
    for index, new_cut_value in enumerate(new_cut):
        ...
        _set_shell_cut_values(parsed_input, species, new_cut_value)

        parsed_input.write(join(rundir, "input.xml"))


def _set_shell_cut_values(parsed_input: ExcitingInputXML, species: str, new_cut_value: float) -> None:
    """Set all DFT-1/2 shell cut values for one species."""
    dfthalfparam = parsed_input.structure.species_properties[species].dfthalfparam
    for shell in dfthalfparam.shell:
        shell.cut = new_cut_value
