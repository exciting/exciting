...
def fit_energy_vs_volume(
    volumes: List[float], energies: List[float], order_of_fit: int, isym: int
) -> Tuple[np.poly1d, List[float], List[float], List[float], float]:
    """
    ...
    for eq_vol in dmin:
        ...
        lattice_const.append(np.cbrt(symmetry_factors[isym] * eq_vol))

    return curve, bulk_modulus, dmin, lattice_const, chi


def print_info_to_stdout(
    isym: int,
    dmin: List[float],
    lattice_const: List[float],
    ...
    print("0 --> Others")
    print("===============================\n")
    print(f"Verification lattice symmetry code      >>>> {isym}")
    print("\n##############################################")
    ...
    for i, eq_vol in enumerate(dmin):
        print(f"\nOptimal volume   =         {eq_vol:17.6f} [Bohr^3]")
        print(
            f"Lattice constant = {symmetry_labels[isym]}        {lattice_const[i]:12.6f} [Bohr]"
        )
        print(f"Bulk modulus     =         {bulk_modulus[i]:17.6f} [GPa]")
        print(f"\nLog(chi)         =          {np.log10(chi):13.2f}")
...
def main() -> None:
    ...
    curv, bulk_modulus, dmin, lattice_const, chi = fit_energy_vs_volume(
        volumes, energies, order_of_polynomial, isym
    )

    # Print results
    print_info_to_stdout(isym, dmin, lattice_const, bulk_modulus, chi)
