...
if tau=="t=1.0":
    ...
    tf=True
K={"SEEBECK_":{True:"$S\ [\mu V/K]$",False:"$S\ [\mu V/K]$"},"ZT_":{True:"$ZT$",False:"$ZT$"},
   "ELECTCOND_":{False:r"$\sigma/\tau$ $[(\Omega cm$ $s)^{-1}]$",True:r"$\sigma$ $[(\Omega cm)^{-1}]$"},
   "THERMALCOND_":{False:r"$\kappa/\tau$ $[W (cm K$ $s)^{-1}]$",True:"$\kappa$ $[W (K cm\ )^{-1}]$"},
   "PF_":{False:r"$\sigma S^2/\tau$ $[\mu W (cm K^2$ $s)^{-1}]$",True:"$\sigma S^2$ $[\mu W/cm$ $K^2]$"}}
