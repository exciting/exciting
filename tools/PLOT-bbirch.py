...
def peos(v,v0,e0,b0,bp):
    v7 = (v0/v)**(7./3)
    v5 = (v0/v)**(5./3)
    v2 = (v0/v)**(2./3)
    ff = v2 - 1.
    bb = bp - 4.
    pp = 3.*b0/2. * (v7-v5) * (1.+3./4*bb*ff) 
    return pp 
...
for i in range(len(xvol)):
    ...
    xpre.append(peos(xvol[i],v0,e0,b0*10,bp))      

pvol = [] ; ppre = []
for i in range(len(volume)-1): 
    pppp=(energy[i+1]-energy[i])/(volume[i]-volume[i+1])
    #if (i==0): print pppp,pppp*unitconv*10
    ppre.append(pppp*unitconv*10)
    pvol.append(volume[i]+(volume[i+1]-volume[i])/2)

bbvol = [] ; bbulk = []
for i in range(len(pvol)-1): 
    bbbbb=(ppre[i+1]-ppre[i])/(pvol[i]-pvol[i+1])
    bvvvv=pvol[i]+(pvol[i+1]-pvol[i])/2
    bbulk.append(bbbbb*bvvvv)
    bbvol.append(bvvvv)
    
bvol = [] ; bulk = []
for i in range(len(xpre)-1): 
    bbbb=(xpre[i+1]-xpre[i])/(xvol[i]-xvol[i+1])
...
#    pppp=(xene[i+1]-xene[i])/(xvol[i]-xvol[i+1])
#    ppre.append(pppp*unitconv*10)
#    pvol.append(xvol[i])
...
plt.plot(bvol,bulk,'b-',label='birch-murnaghan fit')
plt.plot(bbvol,bbulk,'go',label='finite differences')
plt.plot(v0,b0*10,'ro')
