...
ylabel = r'Energy [Ha]'
xlabel = "Interlayer distance [Bohr]"
...
if (narg<2): 
    ...
    print "PLOT-compare-vdW.py FILE DIRECTORY1 DIRECTORY2 ...\n"
    sys.exit()
...
filin = [] ; superx = [] ; supery = [] ; rlabel = []
 
for ifile in range(narg-1): 
    rlabel.append(str(sys.argv[ifile+2]))
    ...
    #y[0] = 0.0
    plt.plot(x,y,asym,label=rlabel[ifile][:15])#"file"+str(ifile+1))
...
plt.legend(loc=1, borderaxespad=0.8, numpoints=1)
