...
def leggi(filin,a,i):
    ...
    y = ifile.readline().strip().split()[i]
    ifile.close()
...
print "\n**Usage**:    PLOT-findtext.py TEXT COLOUMN [DIRECTORYROOT]\n"

if (narg < 2): 
    print "Too few arguments!\n"
    sys.exit()

findtext = str(sys.argv[1])
icoloumn = int(sys.argv[2])

#-------------------------------------------------------------------------------

directoryroot = 'rundir-'
if (narg > 2): directoryroot = str(sys.argv[3])
...
for idir in range(len(list_dir)):
    ...
    r = leggi(fileinp,findtext,icoloumn)
    y.append(float(r))
