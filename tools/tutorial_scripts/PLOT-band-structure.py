...
def check_number_of_plots(nop, nmax):
    ...
    if (nop > nmax):
        ...
        print
        "          Updating the list of colors may be necessary.\n"
    return
...
def main(input_options):
    ...
    for i in range(number_of_plots):
        if (phonon):
            read_phonon_dispersion(kvec, band, infile[i], funit)
        else:
            read_electronic_band(kvec, band, band_spin, spin[i], infile[i], eunit)
