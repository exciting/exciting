...
ylabel  = r'Center of mass (cartesian)'
...
if (narg < 1): 
    ...
    print "PLOT-centerofmass.py DIRECTORYNAME\n"
    sys.exit()
...
os.system("grep \"Center of mass\" "+str(inpf)+" > tempfile")
...
while True:
    ...
    if len(line) == 0: break
    iter+=1
    y1.append(abs(float(line.split()[4])))
    y2.append(abs(float(line.split()[5])))
    y3.append(abs(float(line.split()[6])))
...
plt.plot(x,y3,'go-',label=u'R$_z$')
plt.plot(x,y2,'bo-',label=u'R$_y$')
plt.plot(x,y1,'ro-',label=u'R$_x$')
