...
def test_born_effective_charges():
    ...
    assert np.allclose(zstar_results, zstar_reference,  atol=1.0e-3),(
                       f"energy vs volume values not equivalent to reference calculation. Max err: {abs(zstar_reference - zstar_results).max()}")
