...
def test_dielectric_BN(file_name: str):
...
def test_loss_BN(file_name: str):
    ...
    loss_reference_BN = parse(f"{dirname(__file__)}/{REFERENCE_DIR}/BN/{file_name}")
...
def test_dielectric_TiO2(file_name: str):
    """Test results of dielectric function calculations for rutile titanium dioxide
    """

    epsilon_reference_TiO2 = parse(f"{dirname(__file__)}/{REFERENCE_DIR}/TiO2/{file_name}")
...
def test_loss_TiO2(file_name: str):
    ...
    loss_reference_TiO2 = parse(f"{dirname(__file__)}/{REFERENCE_DIR}/TiO2/{file_name}")
...
def main():
    ...
    max_scf = max([int(i) for i in results_BN['scl'].keys()])
    assert max_scf <= 30, "Expect max 30 SCF iterations to converge"
    converged_results_BN = results_BN['scl'][str(max_scf)]
    test_groundstate_BN(converged_results_BN)

    test_dielectric_BN("EPSILON_BSE-singlet-TDA-BAR_SCR-full_OC11.OUT")

    test_loss_BN("LOSS_BSE-singlet-TDA-BAR_SCR-full_OC11.OUT")
    ...
    test_dielectric_TiO2("EPSILON_BSE-singlet-TDA-BAR_SCR-full_OC11.OUT")

    test_loss_TiO2("LOSS_BSE-singlet-TDA-BAR_SCR-full_OC11.OUT")
