...
REFERENCE_DIR = "reference_05_tutorial_q_dependent_bse_calculations"
...
def test_dielectric_optical_bse(file_name: str):
...
def test_loss_optical_bse(file_name: str):
    ...
    assert np.allclose(loss_optical_bse_results['real_oscillator_strength'],
                       loss_optical_bse_reference['real_oscillator_strength']), \
        f"Real part of loss function not equivalent to reference calculation in optical BSE calculations"

    assert np.allclose(loss_optical_bse_results['imag_oscillator_strength'],
                       loss_optical_bse_reference['imag_oscillator_strength']), \
        f"Imaginary part of loss function not equivalent to reference calculation in optical BSE calculations"


def test_dielectric_xray_bse(file_name: str):
    ...
    assert np.allclose(epsilon_xray_bse_results['real_oscillator_strength_kkt'],
        ...
        f"Real part of dielectric function (by Kramers-Kronig) not equivalent to reference calculation in X-ray BSE" \
        f" calculations"


def test_loss_xray_bse(file_name: str):
    ...
    assert np.allclose(loss_xray_bse_results['real_oscillator_strength'],
                       loss_xray_bse_reference['real_oscillator_strength']), \
        f"Real part of loss function not equivalent to reference calculation in X-ray BSE calculations"

    assert np.allclose(loss_xray_bse_results['imag_oscillator_strength'],
                       loss_xray_bse_reference['imag_oscillator_strength']), \
...
def main():
    ...
    test_dielectric_optical_bse("EPSILON_BSE-singlet-TDA-BAR_SCR-full_OC11.OUT")

    test_loss_optical_bse("LOSS_BSE-singlet-TDA-BAR_SCR-full_OC11.OUT")

    test_dielectric_xray_bse("EPSILON_BSE-singlet-TDA-BAR_SCR-full_OC11.OUT")

    test_loss_xray_bse("LOSS_BSE-singlet-TDA-BAR_SCR-full_OC11.OUT")
