...
Defaults = namedtuple(
    "Defaults",
    [
        "max_time",  # Time after which a test is killed (in seconds)
...
settings = Defaults(
    ...
    SAFE_MODE=bool(os.getenv("SAFE_MODE"))
...
def get_main_output_file_name(method: str) -> str:
    ...
    if method.lower() == 'gw':
        return 'GW_INFO.OUT'
    elif method.lower() == 'phonon':
        return 'PHONON.OUT'
    elif method.lower() == "bse" or method.lower() == "xanes":
        return "INFOXS.OUT"
    return "INFO.OUT"
...
class RunProperties(ABC):
    ...
    @abstractmethod
    def get_additional_output(self) -> str:
        """Get additional information about the run success, if possible. In the case of failure.
...
class ExcitingRunProperties(RunProperties):
    ...
    def get_additional_output(self) -> str:
        """Read the main output file and return the last part of the contents.

        :return: additional information
        """
        main_output = Path(os.path.join(self.run_dir, self.main_output))
        if not main_output.is_file():
            return ""
        content = main_output.read_text()
