...
! correlation energy density
           ...
           & (b1(1)*srs+b2(1)*rs+b3(1)*(srs**3)+b4(1)*rs**(p+1.d0))
            ...
            q1 (2) = 2.d0 * a (2) * &
           & (b1(2)*srs+b2(2)*rs+b3(2)*(srs**3)+b4(2)*rs**(p+1.d0))
            ec1 = q0 (2) * Log (1.d0+1.d0/q1(2))
            q0 (3) = - 2.d0 * a (3) * (1.d0+a1(3)*rs)
            q1 (3) = 2.d0 * a (3) * &
           & (b1(3)*srs+b2(3)*rs+b3(3)*(srs**3)+b4(3)*rs**(p+1.d0))
            ac = - q0 (3) * Log (1.d0+1.d0/q1(3))
            fz = ((1.d0+z)**thrd4+(1.d0-z)**thrd4-2.d0) / &
           & (2.d0**thrd4-2.d0)
            ec (i) = ec0 + ac * (fz/f20) * (1.d0-z4) + (ec1-ec0) * fz * &
           & z4
! exchange potential
...
! correlation potential
            dzf = thrd4 * ((1.d0+z)**thrd-(1.d0-z)**thrd) / &
           & (2.d0**thrd4-2.d0)
            dzec = 4.d0 * (z**3) * fz * (ec1-ec0-ac/f20) + dzf * &
           & ((z4)*ec1-(z4)*ec0+(1.d0-z4)*ac/f20)
            q1p = a (1) * (b1(1)/srs+2.d0*b2(1)+3.d0*b3(1)*srs+2.d0*(p+&
           & 1.d0)*b4(1)*rs**p)
            drec0 = - 2.d0 * a (1) * a1 (1) * Log (1.d0+1.d0/q1(1)) - &
           & q0 (1) * q1p / (q1(1)**2+q1(1))
            q1p = a (2) * (b1(2)/srs+2.d0*b2(2)+3.d0*b3(2)*srs+2.d0*(p+&
           & 1.d0)*b4(2)*rs**p)
            drec1 = - 2.d0 * a (2) * a1 (2) * Log (1.d0+1.d0/q1(2)) - &
           & q0 (2) * q1p / (q1(2)**2+q1(2))
            q1p = a (3) * (b1(3)/srs+2.d0*b2(3)+3.d0*b3(3)*srs+2.d0*(p+&
           & 1.d0)*b4(3)*rs**p)
            drac = 2.d0 * a (3) * a1 (3) * Log (1.d0+1.d0/q1(3)) + q0 &
           & (3) * q1p / (q1(3)**2+q1(3))
            drec = drec0 * (1.d0-fz*z4) + drec1 * fz * z4 + drac * &
           & (fz/f20) * (1.d0-z4)
