...
!$OMP DO   
     ...
     buffer3d=0
     call MPI_allreduce(dvxmt,buffer3d,lmmaxvr*nrcmtmax*natmtot,MPI_DOUBLE_COMPLEX,MPI_SUM,MPI_COMM_WORLD,ierr)
     dvxmt=buffer3d
     deallocate(buffer3d)
     allocate(buffer(ngrtot)) 
     buffer=0
     call MPI_allreduce(dvxir,buffer,ngrtot,MPI_DOUBLE_COMPLEX,MPI_SUM,MPI_COMM_WORLD,ierr)
     dvxir=buffer
     deallocate(buffer)
  if (spinpol) then
     allocate(buffer4d(lmmaxvr,nrcmtmax,natmtot,ndmag))
     buffer4d=0
     call MPI_allreduce(dbxmt,buffer4d,lmmaxvr*nrcmtmax*natmtot*ndmag,MPI_DOUBLE_COMPLEX,MPI_SUM,MPI_COMM_WORLD,ierr)
     dbxmt=buffer4d
     deallocate(buffer4d)
	 
     allocate(buffer2d(ngrtot,ndmag))
     buffer2d=0
     call MPI_allreduce(dbxir,buffer2d,ngrtot*ndmag,MPI_DOUBLE_COMPLEX,MPI_SUM,MPI_COMM_WORLD,ierr)
     dbxir=buffer2d
     deallocate(buffer2d)
	 endif
#endif
