...
! compute the first-variational wavefunctions
    do ist=1, nstfv
      call wavefmt(input%groundstate%lradstep, input%groundstate%lmaxvr, is, ia, ngk(1, ik), apwalm, evecfv(:, ist), &
       lmmaxvr, wfmt1(:, :, ist))
    end do
...
! second-variational Hamiltonian matrix
	...
	do k=1, nsc
	  ...
	  if (i.le.j) then
	    evecsv(i, j) = evecsv(i, j) + zfmtinp(.true., input%groundstate%lmaxmat, nrcmt(is), &
...
! non-collinear
    do ir=1, ngrtot
      bir(ir, :)=(bxcir(ir, :)+ga4*input%groundstate%spin%bfieldc(:))*cfunir(ir)
    end do
  else
! collinear
    do ir=1, ngrtot
      bir(ir, 1:2)=0.d0
      bir(ir, 3)=(bxcir(ir, 1)+ga4*input%groundstate%spin%bfieldc(3))*cfunir(ir)
    end do
  end if
  do jst=1, nstfv
    zfft1(:)=0.d0
