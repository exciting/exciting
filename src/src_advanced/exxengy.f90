...
do is=1, nspecies
  ...
  do ia=1, natoms(is)
	...
	do m2=-spk(jst, is), spk(jst, is)-1
	  call wavefcr(input%groundstate%lradstep, is, ia, jst, m2, nrcmtmax, wfcr2)
	  do ist=1, spnst(is)
	    if (spcore(ist, is)) then
	      do m1=-spk(ist, is), spk(ist, is)-1
		call wavefcr(input%groundstate%lradstep, is, ia, ist, m1, nrcmtmax, wfcr1)
...
! calculate the Coulomb potential
		call zpotclmt(input%groundstate%ptnucl, input%groundstate%lmaxvr, nrc, rcmt(:, is), 0.d0, lmmaxvr, &
		 zrhomt, zvclmt)
		zt1=zfmtinp(.true., input%groundstate%lmaxvr, nrc, rcmt(:, is), lmmaxvr, zrhomt, zvclmt)
