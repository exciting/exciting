!
Subroutine oepvnlk_full (ikp, vnlvv)
      Use modmain
      Use modinput
      Implicit None
! arguments
...
! allocate local arrays
      ...
      Allocate (gqc(ngvec))
      Allocate (jlgqr(0:input%groundstate%lmaxvr+input%groundstate%npsden+1, &
     &  ngvec, nspecies))
...
! pass m-1/2 to wavefcr
                     Call wavefcr (input%groundstate%lradstep, is, ia, ist3, &
                    &  m1, nrcmtmax, wfcr1)
                     Do ist2 = 1, nstsv
! calculate the complex overlap density
                           Call vnlrhomt (.True., is, wfcr1(:,:,1), &
                          &  wfmt1(:,:,ias,1,ist2), zrhomt(:,:,ias))
                           If (associated(input%groundstate%spin)) Then
                              Call vnlrhomt (.True., is, wfcr1(:, :, 2), &
                             &  wfmt1(:, :, ias, 2, ist2), zfmt)
                              zrhomt (:, 1:nrc, ias) = &
                             &  zrhomt (:, 1:nrc, ias) + zfmt (:, 1:nrc)
                           End If
! calculate the Coulomb potential
...
!-------------------------------------------!
! calculate the complex overlap density
                          Call vnlrhomt (.False., is, wfcr1(:,:,1), &
                         &  wfmt1(:, :, ias, 1, ist2), zrhomt(:, :, ias))
                          If (associated(input%groundstate%spin)) Then
