...
! pass m-1/2 to wavefcr
		    call wavefcr(input%groundstate%lradstep, is, ia, ist1, m1, nrcmtmax, wfcr1)
! calculate the complex overlap density
		    ...
		    zt1 = zfmtinp(.false., input%groundstate%lmaxvr, nrc, rcmt(:, is), lmmaxvr, &
		     zrhomt(:, :, ias), zvcltp)
...
!----------------------------------------!
		...
		if (spcore(ist1, is)) then
		  do m2=-spk(ist1, is), spk(ist1, is)-1
		    ic=ic+1
! pass m-1/2 to wavefcr
		    call wavefcr(input%groundstate%lradstep, is, ia, ist1, m2, nrcmtmax, wfcr2)
! calculate the complex overlap density
