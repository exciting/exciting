...
! pass in m-1/2 to wavefcr
	  call wavefcr(input%groundstate%lradstep, is, ia, ist, m, nrcmtmax, wfcr)
	  do jst=1, nstsv
...
! calculate the complex overlap density in the muffin-tin
		...
		zrhomt(:, 1:nrc, ias)=zrhomt(:, 1:nrc, ias)+zfmt(:, 1:nrc)
	      ...
	      zrvx = zfmtinp(.false., input%groundstate%lmaxvr, nrc, rcmt(:, is), lmmaxvr, &
...
! integral of magnetisation dot exchange field
		zmbx=0.d0
		do idm=1, ndmag
		  zmbx = zmbx + zfmtinp(.false., input%groundstate%lmaxvr, nrc, rcmt(:, is), lmmaxvr, &
