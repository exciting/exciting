...
!$omp atomic update
#endif
                    fpdos( ist, iq, l+1, ias) = fpdos( ist, iq, l+1, ias) + dble( dmat( lm, lm, ist, ias))
#ifdef USEOMP
!$omp end atomic
#endif
                  else
              ...
              do ir = 1, wf_kset%nkpt
                if( wfint_mindist) then
                  do ist = 1, wf_nwf
                    do jst = 1, wf_nwf
                      ftweight = zzero
                      do o = 1, wfint_wdistmul( ist, jst, ir)
                        call r3mv( input%structure%crystal%basevect, dble( wfint_wdistvec( :, o, ist, jst, ir)), vr)
                        dotp = dot_product( wfint_kset%vkc( :, iq), vr + wf_centers( :, jst) - wf_centers( :, ist))
                        ftweight = ftweight + cmplx( cos( dotp), sin( dotp), 8)
                      end do
                      ftweight = ftweight/wfint_rmul( ir)/wfint_wdistmul( ist, jst, ir)
                      radcoeffq1( :, ist) = radcoeffq1( :, ist) + ftweight*radcoeffr( jst, :, lm1, ias, ir)*wfint_transform( jst, ist, iq)
                    end do
                  end do
                else
                    ...
                    if( wfint_mindist) then
                      do ist = 1, wf_nwf
                        do jst = 1, wf_nwf
                          ...
                          ftweight = ftweight/wfint_rmul( ir)/wfint_wdistmul( ist, jst, ir)
                          radcoeffq2( :, ist) = radcoeffq2( :, ist) + ftweight*radcoeffr( jst, :, lm2, ias, ir)*wfint_transform( jst, ist, iq)
                        end do
                      end do
                    else
                      call zgemm( 't', 'n', maxdim, wf_nwf, wf_nwf, wfint_pqr( iq, ir), &
...
!$omp end parallel
      ...
      ngkmax = ngkmaxint
      nmatmax = nmatmaxint
      allocate( olp( nmatmax_ptr, nmatmax_ptr))
      allocate( apwalm1( ngkmax_ptr, apwordmax, lmmaxapw, natmtot))
      allocate( apwalm2( ngkmax_ptr, apwordmax, lmmaxapw, natmtot))
      allocate( auxmat( ngkmax_ptr, lmmaxapw))
      allocate( matchc1( apwordmax*lmmaxapw, ngkmax_ptr))
      allocate( matchc2( apwordmax*lmmaxapw, ngkmax_ptr))
      olp = zzero
      apwalm1 = zzero
      apwalm2 = zzero
      ngq_ = wf_Gkset%ngk( 1, iq)
      ngk_ = wf_Gkset%ngk( 1, ik)
      write(*,*) ngk_, ngkmax_ptr
      ...
      deallocate( auxmat)
      allocate( auxmat( nmatmax_ptr, wf_nwf))
      allocate( evectmplo( wf_nwf, wf_nwf))
      allocate( evecfv( nmatmax_ptr, nstfv, nspinor))
      allocate( evecrotkapw( nmatmax_ptr, wf_nwf))
      allocate( evecrotqapw( nmatmax_ptr, wf_nwf))
      ...
      call zgemm( 'n', 'n', ngk_+nlotot, wf_nwf, ngq_+nlotot, zone, &
             olp, nmatmax_ptr, &
             !wfint_evec( :, :, iq), nmatmax_ptr, zzero, &
             evecrotqapw, nmatmax_ptr, zzero, &
             auxmat, nmatmax_ptr)
      call zgemm( 'c', 'n', wf_nwf, wf_nwf, ngk_+nlotot, zone, &
             !wfint_evec( :, :, ik), nmatmax_ptr, &
             evecrotkapw, nmatmax_ptr, &
      ...
      !call zgemm( 'c', 'n', wf_nwf, wf_nwf, ngk_+nlotot, zone, &
      !       evecfv( :, wf_fst:wf_lst, 1), nmatmax_ptr, &
      !       evecfv( :, wf_fst:wf_lst, 1), nmatmax_ptr, zzero, &
      !       evectmplo, wf_nwf)
      !call plotmat( evectmplo)
