...
integer, parameter :: maxsp=104
integer, parameter :: maxspst=40
integer, parameter :: lmax=50
integer spnst,xctype,spnr
integer nrmt,nlx,nlorb
integer is,ist1,ist2,ir,i,l
...
do is=1,maxsp
  ...
  if (i.ne.is) then
    write(*,*)
    write(*,'("Error(species): species mismatch")')
    write(*,'(" required    : ",I8)') is
    write(*,'(" species.dat : ",I8)') i
    write(*,*)
    stop
  end if
  read(40,*) spsymb
  ...
  read(40,*)
  write(*,'("Info(species): running species ",I4,", ",A)') is,trim(spname)
...
! solve the Kohn-Sham-Dirac equations for the atom
    call atom(spzn,spnst,spn,spl,spk,spocc,xctype,np,spnr,r,eval,rho,vr,rwf)
    do ir=spnr,1,-1
...
! open the atomic data file
  ...
  write(50,'(3I4,G14.6,L1,T45,": spn, spl, spk, spocc, spcore")') spn(1), &
   spl(1),spk(1),spocc(1),spcore(1)
  do ist1=2,spnst
