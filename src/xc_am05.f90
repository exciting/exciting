...
! !DESCRIPTION:
!   Calculate the spin-unpolarised exchange-correlation potential and energy of
!   Armiento-Mattsson 05 functional (to be published) for one point.
!
! !REVISION HISTORY:
...
!   the Ceperley-Alder electron gas [Phys. Rev. B 45, 13244 (1992), Phys. Rev.
!   Lett. 45, 566 (1980)]. (Clean room implementation from paper)
!
! !REVISION HISTORY:
!   Created April 2005 (RAR)
!EOP
...
!   Knuth [Adv. Comp. Math. 5, 329-359 (1996)]. The approach is based loosely on
!   that in GNU Octave by N. N. Schraudolph, but this implementation is for real
!   values and the principal branch only.
!
! !REVISION HISTORY:
!   Created April 2005 (RAR)
!EOP
...
! series expansion about -1/e to first order
   val = 1.0d0*sqrt(2.0d0*e*z + 2.0d0) - 1.0d0
endif
! find val through iteration
do i=1,10
   p = exp(val)
   t = val*p - z
   if( val .ne. -1.0d0 ) then
      t = t/(p*(val + 1.0d0) - &
           0.5d0*(val + 2.0d0)*t/(val + 1.0d0))
   else
      t = 0.0d0
   endif
   val = val - t
   if(abs(t) < (2.48d0*1.0d-14)*(1.0d0 + abs(val))) then
      return
   endif
enddo
