...
subroutine wannier_interpolate_density( int_kset)
  ...
  complex(8), allocatable :: evecfv(:,:,:),  evecsv(:,:), apwalm(:,:,:,:,:), wfmtlm(:,:), wfmttp(:,:,:), wfmttp2(:,:)
  ...
  type( k_set) :: tmp_kset, int_kset_red
  real(8), allocatable :: rfft(:,:)
  integer, allocatable :: gfft(:,:), tpr2r(:,:)
  ...
  call generate_k_vectors( int_kset_red, int_kset%bvec, int_kset%ngridk, int_kset%vkloff, .false.)
...
!$OMP END PARALLEL
        ...
        call wfint_putwfmt( ik, ias, wfmttp)
...
!$OMP END PARALLEL
    ...
    call wfint_putwfir( ik, zfft)

  end do
  
  deallocate( apwalm, wfmtlm, wfmttp)
  write(*,*) "wavefunctions written"

  allocate( tpr2r( lmmaxvr, nrcmtmax))
  ir = 0
  do irc = 1, nrcmtmax
    do tp = 1, lmmaxvr
      ir = ir + 1
  ...
  allocate( wfmttp2( lmmaxvr*nrcmtmax, wf_fst:wf_lst))
  ...
  do ik2 = 1, int_kset%nkpt
    !iq = int_kset%ikp2ik( ik2)
    iq = ik2
    wfmttpq = zzero
    do ik = 1, wf_kset%nkpt
      ...
      do ias = 1, natmtot
        ...
        call wfint_getwfmt( ik, ias, wfmttp2)

        call zgemm( 'N', 'N', lmmaxvr*nrcmtmax, wf_nst, wf_nst, zone, &
             wfmttp2, lmmaxvr*nrcmtmax, &
             w, wf_nst, zone, &
             wfmttpq( :, :, ias), lmmaxvr*nrcmtmax)
...
!$OMP DO  
          ...
          do irc = 1, nrcmt( is)
            do tp = 1, lmmaxvr
              z1 = wfmttpq( tpr2r( tp, irc), ist, ias)
              rmttp( tp, irc) = rmttp( tp, irc) + int_kset%wkpt( ik2)*occint( ist, iq)*(dble( z1)**2 + aimag( z1)**2)
            end do
          end do
#ifdef USEOMP                
!$OMP END DO  
...
!$OMP END PARALLEL
  ...
  deallocate( wfmttp2, rmttp, wfmttpq)
