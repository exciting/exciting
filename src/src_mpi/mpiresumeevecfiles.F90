...
allocate(buffer(Recl*recordunit_inbytes))
    ...
    if(rank.lt.nkpt)Open (77, File=outfilenamestring(filetagarg, firstofset(rank, nkpt)), &
        & Action='READ', Form='UNFORMATTED', Access='DIRECT', &
        & Recl=Recl)
    Do ik =1,nkpt
    	if(procofindex(ik, nkpt).eq.rank .and. (rank.lt.nkpt)) then
            Read (77, Rec=ik-firstofset(procofindex(ik, nkpt), nkpt)+1) buffer
        endif
        Call MPI_bcast (buffer,Recl*recordunit_inbytes , MPI_CHARACTER, procofindex(ik, nkpt), MPI_COMM_WORLD, ierr)
