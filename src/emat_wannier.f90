...
! allocate local arrays
      ...
      Allocate (apwalm1(ngkmax, apwordmax, lmmaxvr, natmtot))
      Allocate (apwalm2(ngkmax, apwordmax, lmmaxvr, natmtot))
...
! multiply wavefunction with exp(iq.r)
               ...
               End Do
               Do ist = 1, nst2
                  em (jst, ist) = em (jst, ist) + zfmtinp (.True., &
...
! compute interstitial wavefunctions for k-point k
      Do jst = 1, nst1
         ...
         Do igk = 1, ngk (1, ik)
            ...
            zfir2 (ifg) = evecfv1 (igk, fst1+jst-1)
         End Do
...
! add to the first-variational matrix elements
         Do ist = 1, nst2
            em (jst, ist) = em (jst, ist) + zdotc (ngkq, evecfv2( :, fst2+ist-1), 1, wfir, 1)
         End Do
      End Do
!-------------------------------------------!
...
! get !the second-variational eigenvectors
      ...
      !do is = 1, 4
      !  do ia = 5, nstfv
      !    write( *, '(2I3,3x,SP,E23.16,E23.16,"i")') is, ia-4, em( is, ia)
      !  end do
      !end do
