module m_linalg
  ...
  contains
    ...
    subroutine zgediag( mat, eval, evec)
      ...
      if( info .ne. 0) then
        write(*,'("Error (zgediag): Diagonalization failed. ZGEEV returned info ",i)') info
        stop
      end if
    ...
    subroutine zhediag( mat, eval, evec)
      ...
      if( m .le. 0) then
        write(*,'("Error (zhediag): Invalid matrix dimension (",i,")")'), m
        stop
      end if
      if( size( mat, 2) .ne. m) then
      ...
      if( info .ne. 0) then
        write(*,'("Error (zhediag): Diagonalization failed. ZHEEVR returned info ",i)') info
        stop
      end if
    ...
    subroutine rsydiag( mat, eval, evec)
      ...
      if( info .ne. 0) then
        write(*,'("Error (rsydiag): Tridiagonalization failed. DSYTRD returned info ",i)') info
        stop
      end if
      ...
      if( info .ne. 0) then
        write(*, '("Error( rsydiag): Diagonalisation failed. DSTEQR returned info", i)') info
        stop
      end if
    ...
    subroutine zsvd( mat, sval, lsvec, rsvec)
      ...
      if( info .ne. 0) then
        write(*,'("Error (zsvd): SVD failed. ZGESDD returned info ",i)') info
        stop
      end if
    ...
    subroutine rsvd( mat, sval, lsvec, rsvec)
      ...
      if( info .ne. 0) then
        write(*,'("Error (rsvd): SVD failed. DGESDD returned info ",i)') info
        stop
      end if
