...
!WANNIER
  ...
  integer :: l1, m1
  real(8) :: kc, tp(2), gaunt
  complex(8) :: pf
  complex(8), allocatable :: ylm(:,:)
  real(8), allocatable :: bint(:,:,:)
  external gaunt
!END WANNIER
...
if (input%properties%bandstructure%wannier) then
  ...
  allocate( bint( apwordmax, input%groundstate%lmaxapw, 0:input%groundstate%lmaxvr))
  ...
  allocate( ylm( lmmaxvr, nkpt))
  do ik = 1, nkpt
    Call sphcrd( vkc( :, ik), kc, tp)
    Call genylm( input%groundstate%lmaxvr, tp, ylm( :, ik))
  end do
  DO ik = 1, nkpt
    !============================================
      ...
      do iband = 1, npband
        ...
        do ig = 1, ngk( 1, ik)
          ...
          do l = 0, input%groundstate%lmaxapw
            do io = 1, apword( l, is)
              Call wannier_bessel_int( input%groundstate%lmaxvr, kc, spr( :, is), nr, nr, &
                  & apwfr( :, 1, io, l, ias), lofr( :, 1, projst( iproj, 3), ias), bint( io, l, :)) 
              !write(*,*) bint( io, l, :)
              do m = -l, l
                lm = idxlm( l, m)
                pf = conjg( apwalm( ig, io, lm, ias, 1))*fourpi*exp( -zi*dot_product( vkc( :, ik), atposc( :, ia, is))) 
                do l1 = 0, input%groundstate%lmaxvr
                  do m1 = -l1, l1
                    auxc = auxc + pf*ylm( idxlm( l1, m1), ik)*conjg( zil( l1))*bint( io, l, l1)*gaunt( projst( iproj, 4), l1, l, projst( iproj, 5), m1, m)
                  end do
                end do
              end do
            end do
          end do
          projme( iband, iproj) = projme( iband, iproj) + conjg( evecfv( ig, pbandstart+iband-1, 1))*auxc
        end do
        pf = fourpi*exp( -zi*dot_product( vkc( :, ik), atposc( :, ia, is)))
        do io = 1, nlorb( is)
          l = lorbl( io, is) 
          do m = -l, l
            lm = idxlm( l, m)
            auxc = zzero
            Call wannier_bessel_int( input%groundstate%lmaxvr, kc, spr( :, is), nr, nr, &
                & lofr( :, 1, io, ias), lofr( :, 1, projst( iproj, 3), ias), bint( 1, l, :))
            do l1 = 0, input%groundstate%lmaxvr
              do m1 = -l1, l1
                auxc = auxc + conjg( zil( l1))*ylm( idxlm( l1, m1), ik)*bint( 1, l, l1)*gaunt( projst( iproj, 4), l1, l, projst( iproj, 5), m1, m)
              end do
            end do
            projme( iband, iproj) = projme( iband, iproj) + pf*conjg( evecfv( ngk( 1, ik)+idxlo( lm, io, ias), pbandstart+iband-1, 1))*auxc
          end do
        end do
      end do
      write(*,*) ik, iproj
