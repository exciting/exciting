...
! Vhalf overlap with the auxilliary basis functions
         do ig=0,nsw
         ...
         enddo
! Vhalf is expanded in the auxilliary basis.
...
! Calculate the smooth model Vhalf (we maintain the name rhomodel, the same used in the file rhoinit). 
        do irc=1,nrcmt(is)
          rhotest=0d0
          do ig=0,nsw
...
! Expand the model Vhalf in plane waves.
! To reduce the effort, we express the model Vhalf in terms of the auxilliary basis.
        ffacg=0d0
...
! Calculate Vhalf at each radial grid point on a coarse mesh
...
! Remove model Vhalf from vhalfmt
      Do is = 1, nspecies
        Do ia = 1, natoms (is)
          ias = idxas (ia, is)
          vhalfmt(1,1:nrcmt(is),ias)=vhalfmt(1,1:nrcmt(is),ias)-rhomodel(1:nrcmt(is),is)/y00
        enddo
      enddo

! convert vhalf from a coarse to a fine radial mesh
      Call rfmtctof (vhalfmt)
      Do is = 1, nspecies
         Do ia = 1, natoms (is)
            ias = idxas (ia, is)
            Do ir = 1, nrmt (is)
...
! both things are equivalent. 
         ...
         End Do
      End Do
! interstitial vhalf determined from the atomic tails
