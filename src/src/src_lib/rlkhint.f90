...
! fine-structure constant
      ...
      Real (8) :: rm, ri, tmp1, tmp2,t1,t2,rm0
! automatic arrays
! temporary stuff
...
!         rvr(iter)=vr(iter)
      ...
      p1 (1) = 1.d0
!      rm0 = 1.d0 + 0.5d0 * (alpha**2) *  (energyref-vr(1))
!      rm = rm0/(1d0 - 0.5d0*(e-energyref)*(alpha**2)/rm0)
       rm0 = 1.d0 - 0.5d0 * (alpha**2) *  vr(1)
       rm = rm0/(1d0 - 0.5d0*e*(alpha**2)/rm0)
      If (m .Ne. 0) Then      
! m=1
       q1 (1) = - p0p (1)*(1d0+(alpha**2)*dble (l*(l+1))) / (4.d0*(rm0*r(1))**2)
      else
       q1 (1) = 0d0
      endif
