...
do is=1, nspecies
  ...
  if (iostat.ne.0) then
    ...
    write( * , '("Error(readinput): error opening species file ", A)') &
     trim(input%structure%speciespath)//trim(input%structure%speciesarray(is)%species%speciesfile)
    write(*, *)
    stop
  end if
  read(50, *) input%structure%speciesarray(is)%species%chemicalSymbol
  read(50, *) spname(is)
  read(50, *) spzn(is)
  read(50, *) spmass(is)
