...
module readwritestate
  implicit none
  !tbin,putgithash,putspinorldapublock,putmag,putldapu, &
  !version,githash,present_spin,nspecies,lmmaxvr,nrmtmax,natoms,nrmt,spr,ngrid, &
  !ngvec,ndmag,rhomt,rhoir,vclmt,vclir,vxcmt,vxcir,veffmt,veffir,veffig, &
  !nspinor,ldapu,lmmaxlu,magmt,magir,bxcmt,bxcir,vmatlu)

      integer, parameter :: maxatoms = 10000
      Integer :: natmtot, is
      Logical :: spinpol_
      Integer :: version_ (3), nspecies_, lmmaxvr_, nrmtmax_
      Integer :: natoms_ (maxatoms), ngrid_ (3)
      Integer :: ngrtot_, ngvec_, ndmag_, nspinor_, ldapu_, lmmaxlu_
      Character(40) :: githash_
  ! allocatable arrays
      Integer, Allocatable :: nrmt_ (:)
      ...
      Complex (8), Allocatable :: veffig_ (:)
      Complex (8), Allocatable :: vmatlu_ (:, :, :, :, :)
      logical :: is_nspinor, is_ldapu, is_lmmaxlu, is_githash, is_vmatlu
end module
