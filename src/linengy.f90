...
! find the band energy starting from default
	    ...
	    call findband(input%groundstate%findlinentype,l, 0, input%groundstate%nprad, &
	    nrmt(is), spr(:, is), vr, input%groundstate%deband, &
	     apwe(io1, l, ias))
	  end if
10 continue
	end do
...
! find the band energy starting from default
	    ...
	    call findband(input%groundstate%findlinentype,l, 0, input%groundstate%nprad, nrmt(is),&
