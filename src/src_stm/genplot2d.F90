...
Subroutine genplot2d(fname)
  ...
  If(associated(input%properties%stm%plane)) Then
     ...
     heightl = input%properties%stm%plane%height/zc
     ...
     plot2d_%parallelogram%grid(1)=input%properties%stm%plane%grid(1)
     plot2d_%parallelogram%grid(2)=input%properties%stm%plane%grid(2)
  ...
  Else
     ...
     write(*,'("contain one plane element or one plot2d element.")')
     write(*,*)
     stop
  End If
