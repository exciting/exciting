...
!                                     space summation
      ...
      real(8) :: gausr               ! value of the gaussian function 
!                                      e^(-(rleng/eta)^2)
      real(8) :: pref                ! prefactor for the reciprocal
!                                      lattice sum 
!                                      = 4 pi^(3/2)/v
      real(8) :: gleng               ! the length of gqv
      real(8) :: gqtra               ! the scalar product gqv.raa
      real(8) :: gausg               ! value of the gaussian function 
!                                      e^(-(gleng*eta/2)^2)
      real(8) :: gtolam              ! value of 
!                                      gleng^(lambda-2)/2^(lambda-0.5)
      ...
      real(8), dimension(3) :: g     ! vector belonging to the reciprocal 
!                                      space lattice
      real(8), dimension(3) :: gqv   ! the corresponding sum G+qvec

      complex(8) :: ylam((lambdamax+1)*(lambdamax+1)) ! the values of the spherical 
!                                                       harmonics
      complex(8) :: stmp1((lambdamax+1)*(lambdamax+1)) ! temporary allocation of 
!                                                       the values of sigma
      complex(8) :: stmp2((lambdamax+1)*(lambdamax+1)) ! temporary allocation of 
!                                                       the values of sigma
...
!
     ...
     &                    rleng**(lmbd-2)*gausr/(eta**(2*lmbd-1))
                  do mu=-lmbd,lmbd
                    ...
                    stmp1(lmuind)=stmp1(lmuind)+cmplx(gammaor,0.0d0,8)*     &
     &                           expqdr*ylam(lmuind)
                  enddo ! mu
                enddo ! lmbd
              enddo ! i1
...
!
                ...
                do lmbd=1,lambdamax
                  ...
                  do mu=-lmbd,lmbd
                    lmuind=lmbd*lmbd+lmbd+mu+1
                    stmp2(lmuind)=stmp2(lmuind)+cmplx(pref*gtolam*gausg,    &
     &                           0.0d0,8)*expqdr*ylam(lmuind)*itolam
                  enddo ! mu
                enddo ! lmbd
              enddo !i1
              deallocate(rstr)
              ...
              do lmbd=1,lambdamax
                ...
                enddo
              enddo
              if(ias.eq.jas)sgm(ias,jas,1)=sgm(ias,jas,1)-1.0d0/(eta*pi)
            enddo ! ja
          enddo ! is
        enddo ! ia
      enddo ! is
      ...
      if (DEBUG) then
        open(52,file='STRCONST.OUT',action='WRITE',form='FORMATTED')
        write(52,*)'### eta, rcutoff, gcutoff', eta, rcf, gcf
        write(52,*)'### qvec', qvec
        write(52,*)'-----------------------------------------------------'
        write(52,*)'                    sigma: iq=', iq 
        write(52,*)'-----------------------------------------------------'
        do is = 1, nspecies
          do ia =1, natoms(is)
            ias=idxas(ia,is)
            do js = 1, nspecies
              do ja =1, natoms(js)
                ...
                do lmbd=0,lambdamax
                  do mu=-lmbd,lmbd
                    lmuind=lmbd*lmbd+lmbd+mu+1
                    write(52,10)ias,jas,lmbd,mu,sgm(ias,jas,lmuind)
                  enddo ! mu
                enddo ! lmbd
              enddo ! ja
            enddo ! js
...
! !LOCAL VARIABLES:
      ...
      real(8), dimension(3) :: r     ! vector belonging to the real 
!                                      space lattice
...
!BOC
      ...
      do i=1,3
        lrbs(i)=sqrt(rbas(1,i)*rbas(1,i)+rbas(2,i)*rbas(2,i)+&
     &          rbas(3,i)*rbas(3,i))
      enddo
      lrmin=minval(lrbs)
