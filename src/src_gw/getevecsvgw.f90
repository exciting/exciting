...
subroutine getevecsvgw(fname,ik,vpl,nmatmax,nstsv,nspinor,evecsv)
    use m_getunit
    implicit none
    ! arguments
    character(*) :: fname
    ...
    if (exist) then
      ...
      if (nmatmax_<nmatmax) then
        write(*,*)
        write(*,'("ERROR(getevecsvgw): Wrong number of states")')
      ...
      if (nspinor_<nspinor) then
        write(*,*)
        write(*,'("ERROR(getevecsvgw): Wrong spinor number")')
      ...
      if (t1 > 1.d-6) then
        write(*,*)
        write(*,'("ERROR(getevecsvgw): Differing vectors for k-point",i8)') ik
      ...
      write(*,*)
      write(*,*) "ERROR(getevecsvgw): File ", trim(fname), " does not exist!"
      write(*,*)
      stop
    end if
