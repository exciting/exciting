...
! arguments
  ...
  real(8), intent( in) :: wint(2), e( ld, *), f( ld, *), fmax
  real(8), intent( out) :: g( nw, n)
! local variables
  integer :: mat(8,8), i, j, ist, jst, i3, j1, j2, j3, k1, k2, k3, iw, iw1, iw2, iint, inside(2,2), corner(2,2)
  real(8) :: z, wd, dw, dwi, w(nw), t1, ce(4), cf1(4), cf2(4), a0, x0, x1, y0, y1, a, b, eps, intbound(2,2)
...
#ifdef USEOMP
!$omp parallel default( shared) private( j1, j2, j3, k1, k2, k3, e0, f0, ae, af, ist, jst, i3, z, ce, cf1, cf2, a, b, iw1, iw2, iw, a0, inside, corner, t1, x0, x1, y0, y1, iint, intbound, i, j)
!$omp do collapse(3)
#endif
  do j1 = 0, ngridk(1) - 1
    do j2 = 0, ngridk(2) - 1
      do j3 = 0, ngridk(3) - 1
        ...
        do ist = 1, n
          if( any( f0( ist, :) .gt. eps)) then
          !if( 1) then
          do jst = 1, n
            if( any( f0( jst, :) .lt. fmax-eps) .and. (ist .ne. jst)) then
            !if( 1) then
          
            do i3 = 0, nsk(3) - 1
              z = dble( i3)/dble( nsk(3))
              ce(1) = (ae( jst, 1)-ae( ist, 1)) + (ae( jst, 4)-ae( ist, 4))*z
              ce(2) = (ae( jst, 2)-ae( ist, 2)) + (ae( jst, 6)-ae( ist, 6))*z
              ce(3) = (ae( jst, 3)-ae( ist, 3)) + (ae( jst, 7)-ae( ist, 7))*z
              ce(4) = (ae( jst, 5)-ae( ist, 5)) + (ae( jst, 8)-ae( ist, 8))*z

              cf1(1) = (af( ist, 1)-af( jst, 1)) + (af( ist, 4)-af( jst, 4))*z
              cf1(2) = (af( ist, 2)-af( jst, 2)) + (af( ist, 6)-af( jst, 6))*z
              cf1(3) = (af( ist, 3)-af( jst, 3)) + (af( ist, 7)-af( jst, 7))*z
              cf1(4) = (af( ist, 5)-af( jst, 5)) + (af( ist, 8)-af( jst, 8))*z

              cf2(1) = -af( jst, 1) - af( jst, 4)*z + fmax
              cf2(2) = -af( jst, 2) - af( jst, 6)*z
              cf2(3) = -af( jst, 3) - af( jst, 7)*z
              cf2(4) = -af( jst, 5) - af( jst, 8)*z
              ...
              if( (iw1 .ge. 1) .and. (iw2 .le. nw) .and. (iw1 .le. iw2)) then
                !write(*,'(2(3i,5x),2i,6f13.6)') j1, j2, j3, ist, jst, i3, iw1, iw2, a, b, cf2
                    ...
                    if( abs( ce(4)) .gt. eps) then
                      ...
                      do i = 1, iint
                        if( abs( intbound(2,i) - intbound(1,i)) .gt. eps) then
                          ...
                          a = 0.d0
                          b = ce(4)*ce(4)*cf1(1) - ce(3)*ce(4)*cf1(2) - ce(2)*ce(4)*cf1(3) - a0*ce(4)*cf1(4) + 2.d0*ce(2)*ce(3)*cf1(4)
                          a = a + b*log( abs( (ce(3)+ce(4)*intbound(2,i))/(ce(3)+ce(4)*intbound(1,i)))) 
                          b = ce(4)*ce(4)*cf1(2) - ce(2)*ce(4)*cf1(4)
...
!$omp atomic update
#endif
                          if( abs( a) .gt. eps) g( iw, ist) = g( iw, ist) + t1**3*sign( 1.d0, ce(3)+ce(4)*intbound(j,i))*a
#ifdef USE  OMP
!$omp end atomic
#endif
                        end if
                    ...
                    ! a3 = 0, a2 != 0
                      ...
                      do i = 1, iint
                        if( abs( intbound(2,i) - intbound(1,i)) .gt. eps) then
                          a = 0.d0
                          b = ce(3)*cf1(1) - a0*cf1(3)
                          a = a + b*(intbound(2,i) - intbound(1,i))
                          b = ce(3)*cf1(2) - ce(2)*cf1(3) - a0*cf1(4)
...
!$omp atomic update
#endif
                          if( abs( a) .gt. eps) g( iw, ist) = g( iw, ist) + t1*abs( t1)*a
#ifdef USE  OMP
!$omp end atomic
#endif
                        end if
...
#endif
! normalise function
  t1 = dble( ngridk(1)*ngridk(2)*ngridk(3))*dble( nsk(3))*fmax
  t1 = 1.d0/t1
  g = t1*g
