...
! collinear
	...
	if (xcgrad.le.0) then
	  call xcifc(input%groundstate%xctypenumber, n = lmmaxvr, rhoup = rftp2(:, 1), rhodn = rftp2(:, 2), ex = ex, &
...
!--------------------------!
	call dgemv('N', lmmaxvr, lmmaxvr, 1.d0, rbshtvr, lmmaxvr, rhomt(:, ir, ias), &
	 1, 0.d0, rftp1, 1)
	if (xcgrad.le.0) then
	  call xcifc(input%groundstate%xctypenumber, n=lmmaxvr, rho=rftp1, ex=ex, ec=ec, vx=vx, vc=vc)
	else
	  call xcifc(input%groundstate%xctypenumber, n = lmmaxvr, rho = rftp1, ex = ex, ec = ec, vx = vx, vc = vc, &
...
! collinear
  ...
  if (xcgrad.le.0) then
    call xcifc(input%groundstate%xctypenumber, n = ngrtot, rhoup = rfir(:, 1), rhodn = rfir(:, 2), ex = exir, &
...
!--------------------------!
  if (xcgrad.le.0) then
    call xcifc(input%groundstate%xctypenumber, n=ngrtot, rho=rhoir, ex=exir, ec=ecir, vx=vx, vc=vc)
  else
    call xcifc(input%groundstate%xctypenumber, n = ngrtot, rho = rhoir, ex = exir, ec = ecir, vx = vx, vc = vc, &
