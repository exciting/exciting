...
contains
  ...
  subroutine test_qr_column_pivot_real(test_report)
    !> Test report object
    type(unit_test_type) :: test_report
    real(dp), allocatable :: A(:, :), A_T(:, :), Q(:, :), R(:, :), L(:, :)
...
!
    ...
    ! Test QR factorization for a matrix with N_rows < N_cols. In this case, the QR factorization is not unique but the
    ! LQ factorization with row pivoting is. The LQ factorization with row pivoting can be calculated by calculating the 
    ! QR factorization for the transposed A. Q needs to be transposed and L is the transposed of R. They fullfill the 
    ! following equation:
    !      
    !          A(P, :) = L * Q         
    !
    A = real_matrix_5x7
    A_T = transpose(A)
    call qr_column_pivot(A_T, P, Q, R)
    ...
    L = transpose(R)
    Q = transpose(Q)
    call test_report%assert(all_close(A(P, :), matmul(L, Q)), &
                           'Test if qr_column_pivot for real input with N_rows < N_cols &
                           calculates Q, R and P correctly. &
                           Expected: A (P, :) == L * Q.')
  ...
  subroutine test_qr_column_pivot_complex(test_report)
    !> Test report object
    type(unit_test_type) :: test_report
    complex(dp), allocatable :: A(:, :), A_T(:, :), Q(:, :), R(:, :), L(:, :)
    integer, allocatable :: P(:)
    ...
    ! Test QR factorization for a matrix with N_rows < N_cols. In this case, the QR factorization is not unique but the
    ! LQ factorization is. The LQ factorization with row pivoting can be calculated by calculating the QR factorization 
    ! for the transposed A. Q needs to be transposed and L is the transposed of R. They fullfill the following equation:
    !      
    !          A(P, :) = L * Q         
    !
