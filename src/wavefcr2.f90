...
!
!
Subroutine wavefcr2 (lrstp, is, ia, ist, m, ld, wfcr)
      Use modmain
      Implicit None
! arguments
     ...
     ! Complex (8), Intent (Out) :: wfcr (lmmaxvr, ld, 2)
      Complex (8), Intent (Out) :: wfcr (ntpll, ld, 2)
! local variables
...
! major component of radial wavefunction
	    ...
	    wfcr (1:ntpll, irc, 1) = t2 * zbshthf (1:ntpll, lm)
         End If
         If (Abs(m+1) .Le. l) Then
            lm = idxlm (l, m+1)
            t2 = t1 * cg2
            ! wfcr (1:lmmaxvr, irc, 2) = t2 * zbshtvr (1:lmmaxvr, lm)
	    wfcr (1:ntpll, irc, 2) = t2 * zbshthf (1:ntpll, lm)
         End If
      End Do
      Return
10    Continue
