...
! generate spinor wavefunction from second-variational eigenvectors
	  ...
	  do ispn=1, nspinor
		...
		if (.not.done(ist)) then
		  call wavefmt(1, input%groundstate%lmaxvr, is, ia, ngk(1, ik), apwalm, evecfv(:, ist), &
		   lmmaxvr, wfmt1(:, :, ist))
		  done(ist)=.true.
		end if
...
! spin-unpolarised wavefunction
	  call wavefmt(1, input%groundstate%lmaxvr, is, ia, ngk(1, ik), apwalm, evecfv(:, j), lmmaxvr, &
	   wfmt2)
	end if
! compute the gradient of the wavefunction
	do ispn=1, nspinor
	  call gradzfmt(input%groundstate%lmaxvr, nrmt(is), spr(:, is), lmmaxvr, nrmtmax, &
	   wfmt2(:, :, ispn), gzfmt)
