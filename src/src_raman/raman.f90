...
Logical :: existent, existent1, eq_done, active, nlf
...
!  call construct_symvec
   ...
   do iat = 1, natmtot
      do i = 1, 3
         ic = ic + 1
         ev(ic, 6) = cmplx(input%properties%raman%eigvecarray(ic)%eigvec%comp(1), &
               &           input%properties%raman%eigvecarray(ic)%eigvec%comp(2), 8)
         norm = norm + abs(ev(ic, 6))**2
      enddo
   enddo
...
! check mode if Raman active by symmetry
   write(*,*) 'mode ',ev(:, imode)
   call check_raman (dble(ev(:, imode)), active)
!
...
!        call execute_command_line('mkdir '//trim(prefix))
!        call execute_command_line('cd ./'//trim(prefix))
...
!
      ...
      sfact = 0.5d0 / fgew
!
      if (abs(tempe - tempa) .lt. 1.d-12) then
         ntp = 1
      else
         if (abs(tempi) .lt. 1.d-12) then
            ntp = 1
         else
...
write(66,'(//,116("*"),//," CPU time used in RAMAN: ",f12.2," seconds",/)') finish_time - start_time
! write end
