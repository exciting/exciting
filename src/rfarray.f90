...
real(8)::polynom
external polynom
allocate(ya(input%groundstate%nprad), c(input%groundstate%nprad))
...
! check if point is in a muffin-tin
  do is=1, nspecies
    ...
    do ia=1, natoms(is)
      ...
      v2(:)=v1(:)-atposc(:, ia, is)
      do i1=-1, 1
	v3(:)=v2(:)+dble(i1)*input%structure%crystal%basevect(:, 1)
	do i2=-1, 1
	  v4(:)=v3(:)+dble(i2)*input%structure%crystal%basevect(:, 2)
	  do i3=-1, 1
	    v5(:)=v4(:)+dble(i3)*input%structure%crystal%basevect(:, 3)
