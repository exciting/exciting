...
contains
  ...
  subroutine getemat2(iq,ik,filnam,x1,x2)
    use modmain
    use modxs
    use modmpi
    use m_getunit
    ...
    integer :: recl,un, nstval_,nstcon_,nkpt_,ngq_
    ...
    if (present(x2)) then
       ! I/O record length
       inquire(iolength=recl) nstval_,nstcon_,nkpt_,ngq_,vql_,vkl_,x1,x2
       open(unit=un,file=trim(filnam),form='unformatted',action='read', &
            access='direct',recl=recl)
       read(un,rec=ik) nstval_,nstcon_,nkpt_,ngq_,vql_,vkl_,x1,x2
    else
       ! I/O record length
       inquire(iolength=recl) nstval_,nstcon_,nkpt_,ngq_,vql_,vkl_,x1
       open(unit=un,file=trim(filnam),form='unformatted',action='read', &
            access='direct',recl=recl)
       read(un,rec=ik) nstval_,nstcon_,nkpt_,ngq_,vql_,vkl_,x1
    end if
    close(un)
    ! check consistency
