...
contains
  ...
  subroutine update_density( first_kpt, psi, occupations, it, normalize, l_rad_step, &
    ...
    call assert( psi%n_kpts() == size( occupations, 2 ), 'psi and occupations have different nkpts' )
    ...
    !$OMP SHARED(first_kpt, psi, occupations_slice, dens_case_, rhomt, rhoir)
    !$OMP DO
    do i = 1, size( occupations_slice, 2 )
      ik = first_kpt + i - 1
      select case ( dens_case_ )
      case( active_and_frozen )
        call rhovalk( ik, psi%active(:, :, i), occupations_slice(:, i), rhomt )
        call genrhoir( ik, psi%active(:, :, i), occupations_slice(:, i), rhoir )
      case( save_and_frozen )
        call rhovalk( ik, psi%active_save(:, :, i), occupations_slice(:, i), rhomt )
        call genrhoir( ik, psi%active_save(:, :, i), occupations_slice(:, i), rhoir )
      case( frozen )
        call rhovalk( ik, psi%frozen(:, :, i), occupations_slice(:, i), rhomt )
        call genrhoir( ik, psi%frozen(:, :, i), occupations_slice(:, i), rhoir )
      case( groundstate )
        call rhovalk( ik, psi%groundstate(:, :, i), occupations_slice(:, i), rhomt )
        call genrhoir( ik, psi%groundstate(:, :, i), occupations_slice(:, i), rhoir )
      end select
    end do
    !$OMP END PARALLEL
