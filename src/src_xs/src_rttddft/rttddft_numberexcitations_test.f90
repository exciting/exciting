...
contains
  ...
  subroutine rttddft_NumberExcitations_test_driver( mpiglobal, kill_on_failure)
    ...
    integer(i32), parameter :: n_assertions_test_obtain_number_excitations = 4
  ...
  subroutine test_obtain_number_excitations( mpiglobal, test_report )
    ...
    complex(dp), allocatable :: psi(:, :, :), psi_gnd(:, :, :), tmp(:, :), proj(:, :)
    ...
    allocate( psi(n_dim, n_states, n_kpt), psi_gnd(n_dim, n_states_gnd, n_kpt) )
    ...
    do ik = 1, n_kpt
      ...
      call solve_generalized_hermitian_eigenproblem( H, S, tol, eigs, psi(:, :, ik) )
    ...
    do ik = 1, n_kpt
      ...
      proj = matmul( conjg( transpose( tmp ) ), psi(:, :, ik) )
    ...
    call obtain_number_excitations( psi_gnd(:, :, first_k:last_k), psi(:, :, first_k:last_k), &
      overlap(:, :, first_k:last_k), tol, occ_gnd(:, first_k:last_k), wkpt(first_k:last_k), mpiglobal, n_exc, n_gs )
    ...
    call obtain_number_excitations( psi_gnd(:, :, first_k:last_k), psi_gnd(:, :, first_k:last_k), &
      overlap(:, :, first_k:last_k), tol, occ_gnd(:, first_k:last_k), wkpt(first_k:last_k), mpiglobal, n_exc, n_gs )
