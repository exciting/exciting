...
subroutine tetcalccwq2(iq)
  ...
  ! local variables
  character(*), parameter :: thisnam='tetcalccwq2'
...
!!$  call findocclims(iq,istocc0,istocc,istunocc0,istunocc,isto0,isto,istu0,istu)
  ...
  allocate(cwsurft2(nstsv,nstsv),cwt2(nstsv,nstsv),cwat2(nstsv,nstsv))
  allocate(w(nwdf))
  ...
  call getunit(un)
  inquire(iolength=recl) cw(:,:,1),cwa(:,:,1),cwsurf(:,:,1)
...
write(*,*) 'tetcalccwq2: 1st loop:21/1/5',cwsurf(1,5,ik),cwsurf(5,1,ik)
  ...
  allocate(cw(nwdfp,nstsv,nstsv))
  allocate(cwa(nwdfp,nstsv,nstsv))
  allocate(cwsurf(nwdfp,nstsv,nstsv))
