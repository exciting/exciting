...
module m_getgrst
  contains
    ...
    subroutine wavefmt0(lrstp, lmax, is, ia, ngp, apwalm, evecfv, ld, wfmt)
    ...
    end subroutine getevecsv1
!--------------------------------------------------------------------------------
  subroutine wavefmtsv1(lrstp ,lmax ,is ,ia , ngp, ist1, apwalm, evecfv, evecsv, wfmtsv)
      ...
      do ist = 1, nstfv
        ...
        if (abs(zt1)>1.d-8) then
          if (.not.done(ist)) then
            call wavefmt1(lrstp, lmax, is, ia, &
    ...
    return
 end subroutine wavefmtsv1
!--------------------------------------------------------------------------------
  subroutine wavefmtsv0(lrstp ,lmax ,is ,ia , ngp, ist1, apwalm, evecfv, evecsv, wfmtsv)
    ...
    do ispn = 1, nspinor
      do ist = 1, nstfv
        ...
        if (abs(zt1)>1.d-8) then
          if (.not.done(ist)) then
            call wavefmt0(lrstp, lmax, is, ia, &
              & ngp, apwalm, evecfv(:,ist), lmmaxapw, wfmt1(:,:,ist))
            done(ist) = .true.
          end if
          ! add to spinor wavefunction
    ...
    deallocate(done, wfmt1)
    return
 end subroutine wavefmtsv0
!--------------------------------------------------------------------------------
  subroutine wavefmtsv(lrstp ,lmax ,is ,ia , ngp, ist1, apwalm, evecfv, evecsv, wfmtsv)
