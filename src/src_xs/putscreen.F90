...
subroutine putscreen(un,tq0,n,chi0h,chi0w,chi0)
  use modxs, only: krondelta
  implicit none
  ! input parameters
  logical, intent(in) :: tq0
  integer, intent(in) :: un,n
  complex(8), intent(in) :: chi0h(3,3),chi0w(n,2,3),chi0(n,n)
  ! local variables
  integer :: ig1,ig2,i,j
  real(8) :: r1
    do ig1=1,n
        ...
        if (tq0) then
           if ((ig1.eq.1).and.(ig2.eq.1)) then
              write(un,'(2i8,2g18.10)') ((-i,-j,dble(krondelta(i,j))- &
                   chi0h(i,j),j=1,3),i=1,3)
           end if
           if ((ig1.eq.1).and.(ig2.ne.1)) then
              write(un,'(2i8,2g18.10)') (-i,ig2,-chi0w(ig2,1,i),i=1,3)
           end if
           if ((ig1.ne.1).and.(ig2.eq.1)) then
              write(un,'(2i8,2g18.10)') (ig1,j,-chi0w(ig1,2,j),j=1,3)
           end if
           if ((ig1.ne.1).and.(ig2.ne.1)) then
              write(un,'(2i8,2g18.10)') ig1,ig2,r1-chi0(ig1,ig2)
           end if
        else
           write(un,'(2i8,2g18.10)') ig1,ig2,r1-chi0(ig1,ig2)
        end if
