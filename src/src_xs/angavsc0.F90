...
! !ROUTINE: angavsc0
! !INTERFACE:
subroutine angavsc0(n, nmax, scrnh, scrnw, scrn, scieff, eps_inv_head, eps_inv_wings)
  ! !USES:
    use modmpi
  ...
  !BOC
    ...
    !> Averaged head of inverse dielectric matrix at the Gamma point
    complex(8), intent(out) :: eps_inv_head
    !> Averaged wings of the dielectric matrix at the Gamma point
    complex(8), intent(out) :: eps_inv_wings(n-1,2)
    ...
    select case(trim(input%xs%bse%sciavtype))
      ...
      case('spherical')
        ...
        eps_inv_head = t00 *dot_product(mxxlm, ei00lm)
        ...
        fullinv: if(input%xs%bse%scrherm .eq. 0) then
          ...
          do j1 = 2, n
            ...
            eps_inv_wings(j1 - 1, 1) = t00*dot_product(mxxlm, ei0xlm)
            eps_inv_wings(j1 - 1 ,2) = t00*dot_product(mxxlm, eix0lm)
           

            ! ! if(j1 == 3) write(*, '(A10, 2F10.5)') 'eps013', t00 *dot_product(mxxlm, ei0xlm) 
            ! ! if(j1 == 3) write(*, '(A10, 2F10.5)') 'w013', scieff(1, j1)
