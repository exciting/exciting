...
write( * , '(a, i5, 2g18.10)') 'input%properties%linresponsetensor%optcomp,    1/eps_00(q = 0)	    :', &
	     oct, 1.d0/scrnh0(oct)
write( * , '(a, i5, 2g18.10)') 'input%properties%linresponsetensor%optcomp,	 eps_00(q = 0)	    :', &
	     oct, scrnh0(oct)

	call zinvert_hermitian(input%xs%BSE%scrherm, tm, tmi)
	scri(1:n, 1:n)=tmi(:, :)
        ! keep head of inverse dielectric matrix for q=0
	scrnih0(oct)=scri(1, 1)

write( * , '(a, i5, 2g18.10)') 'input%properties%linresponsetensor%optcomp,   eps^ - 1_00(q = 0)      :', &
     oct, scrnih0(oct)
write( * , '(a, i5, 2g18.10)') 'input%properties%linresponsetensor%optcomp, 1/eps^ - 1_00(q = 0)      :', &
...
write( * , '(a, i5, 2g18.10)') 'input%properties%linresponsetensor%optcomp, symm.   eps^ - 1_00(q = 0):', &
     iqr, scri(1, 1)
write( * , '(a, i5, 2g18.10)') 'input%properties%linresponsetensor%optcomp, symm. 1/eps^ - 1_00(q = 0):', &
	  iqr, 1.d0/scri(1, 1)
