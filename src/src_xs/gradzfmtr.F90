...
! !ROUTINE: gradzfmtr
! !INTERFACE:
subroutine gradzfmtr(lmax,nr,r,ld1,ld2,fmt,gfmt)
...
!   fmt  : real muffin-tin function (in,real(ld1,nr))
     ...
     do m1=-l1,l1
        ...
        if (l2.le.lmax) then
           ...
           do m2=-l2,l2
              ...
              do ir=1,nr
                 tt1=g1(ir)
                 tt2=t1*(fmt(lm1,ir)*dble(l1)/r(ir)-tt1)
        ...
        if (l2.ge.0) then
           ...
           do m2=-l2,l2
              ...
              do ir=1,nr
                 tt1=g1(ir)
                 tt2=t2*(fmt(lm1,ir)*dble(l1+1)/r(ir)+tt1)
                 gfmt(lm2,ir,1)=gfmt(lm2,ir,1)+((t3-t5)/sqtwo)*tt2
                 gfmt(lm2,ir,2)=gfmt(lm2,ir,2)+((-t3-t5)/sqtwo)*tt2
                 gfmt(lm2,ir,3)=gfmt(lm2,ir,3)+t4*tt2
              end do
