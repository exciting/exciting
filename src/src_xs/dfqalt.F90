...
!   ({\bf q},\omega)$ for one ${\bf q}$-point. Alternative implementation
!   that has the loop over energies as the outer one. For details see routine
!   {\tt dfq}.
!   This routine can be run with MPI parallelization for energy-points.
!
! !REVISION HISTORY:
!   Created August 2008 (Sagmeister)
!EOP
!BOC
  implicit none
  ! arguments
  ...
  ! local variables
  character(*), parameter :: thisnam='dfqalt'
  ...
  integer :: n,j,i1,i2,j1,j2,ik,ikq,igq,igqp,iw,wi,wf,ist1,ist2,nwdfp
  ...
  do ik=1,nkpt
     ...
     ! store energy differences with k-point index

     ! read and store matrix elements for (ik,igq) accessibility
  ...
  do iw=wi,wf    
     ...
     do igq=1,n
        do igqp=1,n
           ...
           ! weight_vck(w) * M_vck =: N_vck(w), for all vck

           ! dot product: < N_vck(w) , M_vck > ; use ZDOTC


           ! end loop over (G,G') pairs
        end do
     end do
