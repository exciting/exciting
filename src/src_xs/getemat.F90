...
contains
  ...
  subroutine getemat(iq,ik,tarec,n1,n2,n3,n4,ngp,filnam,x1,x2)
    ...
    if (lerr) then
       ...
       write(unitout,'(" array size2 for G+q vectors     : ",2i6)') size(x1,3),&
            size(x2,3)
       if (present(x2)) then
    ...
    if (present(x2)) then
       ...
       read(un,rec=1) vql_,vkl_,nstsv_,ngq_,n1_,n2_,n3_,n4_
       close(un)
    else
       ! I/O record length
       inquire(iolength=recl) vql_,vkl_,nstsv_,ngq_,n1_,n2_
       open(unit=un,file=trim(filnam),form='unformatted',action='read', &
            access='direct',recl=recl)
       read(un,rec=ikr) vql_,vkl_,nstsv_,ngq_,n1_,n2_
       close(un)
    end if
    err=0
    ! check block sizes
    lerr=(n1.gt.n1_).or.(n2.gt.n2_).or.(ngq(iq).gt.ngq_)
    if (present(x2)) lerr=lerr.or.(n3.gt.n3_).or.(n4.gt.n4_)
    if (lerr) then
       write(unitout,*)
       write(unitout,'("Error(",a,"): inconsistent block sizes:")') &
            thisnam
       if (present(x2)) then
          write(unitout,'(" requested band block sizes : ",4i6)') n1,n2,n3,n4
          write(unitout,'(" band block sizes from file : ",4i6)') n1_,n2_,n3_, &
               n4_
       else
          write(unitout,'(" requested band block sizes : ",2i6)') n1,n2
          write(unitout,'(" band block sizes from file : ",2i6)') n1_,n2_
       end if
       write(unitout,*)
       call flushifc(unitout)
       err=err+1 
    ...
    !     get data     !
    !------------------!
    allocate(x1t(n1_,n2_,ngq_))
    if (present(x2)) allocate(x2t(n3_,n4_,ngq_))
    call getunit(un)
    if (present(x2)) then
       ! I/O record length
       inquire(iolength=recl) vql_,vkl_,nstsv_,ngq_,n1_,n2_,n3_,n4_, &
       ...
       open(unit=un,file=trim(filnam),form='unformatted',action='read', &
            access='direct',recl=recl)
       read(un,rec=ikr) vql_,vkl_,nstsv_,ngq_,n1_,n2_,n3_,n4_,x1t,x2t
    else
       ! I/O record length
       inquire(iolength=recl) vql_,vkl_,nstsv_,ngq_,n1_,n2_,x1t
       open(unit=un,file=trim(filnam),form='unformatted',action='read', &
            access='direct',recl=recl)
       read(un,rec=ikr) vql_,vkl_,nstsv_,ngq_,n1_,n2_,x1t
    end if
    close(un)
    ! check consistency
    ...
    ! retreive data within cutoff
    x1(:,:,:)=x1t(:n1,:n2,:ngp)
    deallocate(x1t)
    if (present(x2)) x2(:,:,:)=x2t(:n3,:n4,:ngp)
    if (present(x2)) deallocate(x2t)
