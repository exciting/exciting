...
use modmain
! !DESCRIPTION:
!   Generates a set of ${\bf G}$-vectors for the Fourier transform of the charge
!   density and potential, subject to the condition $|{\bf G}|\le G_{\rm max}$,
!   and sorts them according to length. Integers corresponding to the vectors
!   in lattice coordinates are stored, as well as the map from these integer
!   coordinates to the ${\bf G}$-vector index. A map from the ${\bf G}$-vector
!   set to the standard FFT array structure is also generated.
!
! !REVISION HISTORY:
...
allocate(gc(ngvec))
! collect all G-vectors with |G| < gmaxvr
t1=gmaxvr**2
ig=0
