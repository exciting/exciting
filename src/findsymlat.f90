...
call r3mtm(input%structure%crystal%basevect, input%structure%crystal%basevect, g)
...
! check invariance of metric tensor
  ...
  do i=1, 3
    do j=1, 3
      if (abs(sgs(i, j)-g(i, j)).gt.input%structure%epslat) goto 10
    end do
  end do
...
do i=1, nsymlat
  s(:, :)=dble(symlat(:, :, i))
  call r3mm(s, ainv, c)
  call r3mm(input%structure%crystal%basevect, c, symlatc(:, :, i))
end do
return
end subroutine
!EOC
