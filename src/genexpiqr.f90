...
t1 = input%properties%elnes%vecql(1) ** 2 + input%properties%elnes%vecql(2) ** 2 + input%properties%elnes%vecql(3) ** 2
...
! calculate the wavefunction for k-point k+q
      call wavefmt(input%groundstate%lradstep, input%groundstate%lmaxvr, is, ia, ngkq, apwalm2, evecfv2(:, ist), &
       lmmaxvr, wfmt2(:, :, ist))
    end do
    do jst=1, nstfv
! calculate the wavefunction for k-point k
      call wavefmt(input%groundstate%lradstep, input%groundstate%lmaxvr, is, ia, ngk(1, ik), apwalm1, evecfv1(:, jst),&
   lmmaxvr, wfmt1)
! multiply wavefunction with exp(iq.r)
	...
	wfmt3(:, irc)=zflm(:)
      ...
      do ist=1, nstfv
	em(ist, jst) = em(ist, jst) + zfmtinp(.true., input%groundstate%lmaxvr, nrcmt(is), rcmt(:, is), &
...
write(8899, '("# Q=q+Gq point", 3g18.10)') input%properties%elnes%vecql(:)
