	...
	<xsl:template match="/">
		...
		<xsl:if test="/input/structure/species">
			...
			<xsl:for-each select="/input/structure/species">
				<xsl:text> '</xsl:text>
				<xsl:value-of select="/input/structure/species/@abrev" />
				<xsl:text>'
  </xsl:text>
		...
		<xsl:for-each select="//@*">
			<xsl:choose>
				...
				<xsl:when test="name(.)='stretch'">
					...
					<xsl:variable name="scale3"
						select="substring-after(normalize-space(.),concat($scale1,' ',$scale2,' '))" />
