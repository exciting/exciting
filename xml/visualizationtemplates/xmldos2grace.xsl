...
@    title "</xsl:text><xsl:value-of select="/dos/title"/><xsl:text>"
@    title font 4
@    title size 1.80000
@    title color 1
@    subtitle ""
...
@    frame background pattern 0
   ...
   <xsl:for-each select="exsl:node-set($content)/dos/*/diagram">
      <xsl:variable name="s0">
         <xsl:if test="@nspin=1"><xsl:value-of select="'\f{Symbol}\c-\f{}\C'"/></xsl:if>
         <xsl:if test="@nspin=2"><xsl:value-of select="'\f{Symbol}\c/\f{}\C'"/></xsl:if>
      </xsl:variable>
      <xsl:choose>
         <xsl:when test="../@type='partial'">
<xsl:text>
@s</xsl:text><xsl:value-of select="position()"/><xsl:text> legend "</xsl:text><xsl:value-of select="../@speciessym"/>
<xsl:text>-</xsl:text><xsl:value-of select="../@atom"/><xsl:text> (l=</xsl:text><xsl:value-of select="@l"/>
<xsl:text>, m=</xsl:text><xsl:value-of select="@m"/><xsl:text>, </xsl:text><xsl:value-of select="$s0"/><xsl:text>)"</xsl:text>
         </xsl:when>
         <xsl:otherwise>
            <xsl:if test="@type='totaldos'">
<xsl:text>
@s</xsl:text><xsl:value-of select="position()"/><xsl:text> legend "total </xsl:text><xsl:value-of select="$s0"/><xsl:text>"</xsl:text>
            </xsl:if>
            <xsl:if test="@type='interstitial'">
<xsl:text>
@s</xsl:text><xsl:value-of select="position()"/><xsl:text> legend "interstitial </xsl:text><xsl:value-of select="$s0"/><xsl:text>"</xsl:text>
            </xsl:if>
         </xsl:otherwise>
      </xsl:choose>
   </xsl:for-each>
