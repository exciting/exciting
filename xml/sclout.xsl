...
<xsl:value-of select="$ymin"/><xsl:text>,</xsl:text><xsl:value-of select="$ymax"/>
	...
	<xsl:template match="/">
	 ...
	 <xsl:element name="META">
		...
		<xsl:when test="/groundstate/scl/iter/timing/@itertime">
		<xsl:value-of select="ceiling(/groundstate/scl/iter/timing/@itertime[last()])"/>
		</xsl:when>
		<xsl:otherwise>
		...
		<xsl:value-of select="groundstate/scl/iter/charges/atom/@species"/> scl i=<xsl:value-of select="count(groundstate/scl[last()]/iter)"/>
		<xsl:if test="groundstate/structure"> relax i=<xsl:value-of select="count(groundstate/structure)"/> </xsl:if>
			...
			<body>
				...
				<xsl:call-template name="agraph">
				<xsl:with-param name="data" select="/groundstate/scl[last()]/iter/energies/@totalEnergy" />
		    	...
		    	<xsl:attribute name="src">
				<xsl:call-template name="agraph">
				<xsl:with-param name="data" select="/groundstate/scl[last()]/iter/@rmslog10" />
			    ...
			    <xsl:attribute name="src">
				<xsl:call-template name="agraph">
				<xsl:with-param name="data" select="/groundstate/scl[last()]/iter/charges/@core_leakage" />
			    ...
			    <xsl:attribute name="src">
				<xsl:call-template name="agraph">
				<xsl:with-param name="data" select="/groundstate/scl[last()]/iter/energies/@fermiEnergy" />
				<xsl:with-param name="title" select="'Fermi energy'" />
				</xsl:call-template>
				</xsl:attribute>
				</xsl:element>			
				<xsl:if test="/groundstate/structure/@forceMax">
				<xsl:element name="img">
			    <xsl:attribute name="src">
				<xsl:call-template name="agraph">
				<xsl:with-param name="data" select="/groundstate/structure/@forceMax" />
        ...
        <table><tr><td valign = "top">
				<xsl:for-each select ="/groundstate/scl[last()]/iter[last()]">
			    ...
			    <xsl:if test="/groundstate/structure[last()]/species/atom/forces">
			    <xsl:element name="div">
					<xsl:attribute name="style"/>
				
					<xsl:for-each select="/groundstate/structure[last()]/species">
          ...
          <xsl:attribute name="href">
        <xsl:call-template name="agraph">
        <xsl:with-param name="data" select="/groundstate/structure/species[@chemicalSymbol=$chemicalSymbol]/atom[position()=$anr]/forces/@Magnitude" />
        ...
        <xsl:value-of select="@chemicalSymbol"/><xsl:text>+atom+</xsl:text>
         <xsl:value-of select="$anr"/><xsl:text>+Force+Magnitude</xsl:text>
          ...
          <td><xsl:value-of select="forces/@Magnitude"/></td>
					...
					<xsl:attribute name="href">
				 <xsl:call-template name="agraph">
				<xsl:with-param name="data" select="/groundstate/structure/species[@chemicalSymbol=$chemicalSymbol]/atom[position()=$anr]/forces/*[name()=$forcename]/@*[name()=$attname]" />
				<xsl:with-param name="title" ><xsl:text>species+</xsl:text>
				<xsl:value-of select="$chemicalSymbol"/><xsl:text>+atom+</xsl:text>
...
<xsl:value-of select="$forcename"/><xsl:text>+component+</xsl:text>
				...
				<xsl:with-param name="size" select="'600x300'" />
					...
					<xsl:attribute name="href">
					<xsl:call-template name="agraph">
				<xsl:with-param name="data" select="/groundstate/structure/species[@chemicalSymbol=$chemicalSymbol]/atom[position()=$anr]/@*[name()=$attname]" />
				<xsl:with-param name="title" ><xsl:text>species+</xsl:text>
				<xsl:value-of select="$chemicalSymbol"/><xsl:text>+atom+</xsl:text>
<xsl:value-of select="$anr"/><xsl:text>+</xsl:text>
<xsl:text>position+component+</xsl:text>
				...
				<xsl:with-param name="size" select="'600x300'" />
					...
					<xsl:if test="/groundstate/scl/iter/moments">
          <xsl:element name="div">
          <h2>Moments</h2>
          <table>
          <xsl:for-each select="/groundstate/scl/iter[last()]/moments/*[name()!='atom']">
          ...
          <xsl:variable name="attname"><xsl:value-of select="name(.)"/> </xsl:variable>
           ...
           <xsl:attribute name="href">
           <xsl:call-template name="agraph">
        <xsl:with-param name="data" select="/groundstate/scl[last()]/iter/moments/*[name()=$momentname]/@*[name()=$attname]" />
...
<xsl:text>+component+</xsl:text>
        ...
        <xsl:with-param name="size" select="'600x300'" />
          ...
          <xsl:for-each select="/groundstate/scl/iter[last()]/moments/atom">
          ...
          <xsl:for-each select="@*">
           ...
           <xsl:attribute name="href">
           <xsl:call-template name="agraph">
        <xsl:with-param name="data" select="/groundstate/scl[last()]/iter/moments/atom[$anr]/*[name()=$momentname]/@*[name()=$attname]" />
        <xsl:with-param name="title" ><xsl:text>species+</xsl:text>
        <xsl:value-of select="@chemicalSymbol"/><xsl:text>+atom+</xsl:text>
<xsl:value-of select="$anr"/><xsl:text>+</xsl:text>
 <xsl:value-of select="$momentname" /> 
